...
def send_telegram_photo(chat_id, photo_bytes, token, caption=""):
    """Sends a photo via Telegram Bot API."""
...
def _get_user_specific_report_data(start_date_local, end_date_local, db, user_settings_doc):
    ...
    # Dates
    start_utc = datetime.combine(start_date_local, time.min, tzinfo=PHNOM_PENH_TZ).astimezone(UTC_TZ)
    end_utc = datetime.combine(end_date_local, time.max, tzinfo=PHNOM_PENH_TZ).astimezone(UTC_TZ)
    ...
    facet_res = list(db.transactions.aggregate(build_faceted_report_pipeline(date_match, user_match, user_rate)))
    facets = facet_res[0] if facet_res else {}
    op_data = facets.get('operational', [])
                ...
                try:
                    ...
                    if data['summary']['totalExpenseUSD'] > 0 or data['summary']['totalIncomeUSD'] > 0:
                        ...
                        if chart := _create_chart(data, start_date, end_date):
                            send_telegram_photo(chat_id, chart, Config.TELEGRAM_TOKEN)
                except Exception as e:
...
def send_daily_reminder_job():
    try:
        ...
        for user in users:
            try:
                uid = user['account_id']
                # Check if any transaction exists for today
